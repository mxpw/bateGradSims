...
Correlated paternity can be computed from fertilized_eggs list - the __correlated_paternity()__ function returns patternity correlations by female, while __pop_average_rp()__ returns the pop. level average patternity correlation (raw or corrected, following Dorken & Perry, 2017).
...
With prorata strategy, plot showing upsampling effects can be displayed (using __upsampling_plot__ parameter) - it shows, for each the female, the initial number of eggs to sample (black dots), the minimal threshold (black dashed horizontal line), and the number of eggs to sample after upsampling (red dots). Crosses on the right indicate females that will be remove (because __min_threshold__ can't be reach, even with upsampling). 

Once sampled eggs are available (__sampled_fertilized_eggs__ here), sexual selection base variables can be computed using __get_sexual_selection_components()__ function. It will return MS/RS for females and males. Note that paternity share can be (and is, by default) considered. Both __fertilized_eggs__ (before sampling) and __sampled_fertilized_eggs__ (after sampling) should be provided since females RS depends on eggs counts before sampling (and this need to be know to compute male RS while considering paternity share). 

__Note__ : some parameters combinations will rise errors (typically when requested total number of eggs can't be reached) - as it is, it will stop the job. I'm not sure it's the best behaviour (from a "security" viewpoint, it seems better instead of doing weird and not really controlled stuffs, but, from an automatisation viewpoint, it might break down pipeline - which can be boring when running batch of jobs...). 
...
Returned statustucs are; observed mating success (mean and sd, mso_mean and mso_sd resp.) - computed from __mso()__ function (i.e. computed from where pollen land), genetic mating success (mean and sd, msg_mean and msg_sd) computed after sampling (except when sampling_method is <base>, in which case, values displayed correspond to "true" genetic mating success - number of mate after pollen competition but before sampling) and lastly, reproductive success (rsg_mean and rsg_sd). 
...
Lastly, Bateman gradients can be estimated in turn from output of the sampling method; MS and RS can be scaled (using weird but apparently correct ; $\tilde x = \frac{x}{\bar x}$, with the __scaled__ parameter, which is True by default), and GLM family can also be specified (Gaussian by default). Any family accepted by lme4::glm() can be specify (e.g. poisson, quasipoisson). (might implement more options in future, e.g. negative binomial ?). If __n_gam__ is available in the dataframe (by default __sampling()__ function return those, although it might not be known for males in real situations), additional partial Bateman gradients are also computed (i.e. rs ~ (ms + n_gam) * sex). 
...
__Note__ ; as it is, no R2 is returned since we implement GLMs (check whether it make sense to add id - for Gaussian model, no question, but what about Poisson ? see e.g. [Stack](https://stats.stackexchange.com/questions/551915/interpreting-nonlinear-regression-r2), we could use Pseudo-R2, is it what we want ?)
...
__Temporary plots__ ; have to work a bit on those (not really usefull for simulation purpose)
